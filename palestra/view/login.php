...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['azione']) && $_POST['azione'] === 'registra') {
    ...
    $giorni_settimana // i (intero!)
            ...
            if ($stmt->execute()) {
                ...
                $ins_peso = $db->prepare("INSERT INTO pesi (id_utente, peso, data) VALUES (?, ?, ?)");
                $ins_peso->bind_param("ids", $nuovo_id, $peso, $data_oggi);
                $ins_peso->execute();
                
                // Genera la scheda di allenamento personalizzata
                require_once $_SERVER['DOCUMENT_ROOT'] . '/palestra/controller/GeneraSchedaDefault.php';
