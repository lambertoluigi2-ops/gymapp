...
$db       = connetti();
$oggi_num = (int)date('N'); // 1=Lunedì … 7=Domenica

// Scheda attiva dell'utente
$rs = $db->prepare("SELECT id_scheda FROM schede WHERE id_utente=? AND attiva=1 ORDER BY created_at DESC LIMIT 1");
$rs->bind_param("i", $uid);
$rs->execute();
$scheda = $rs->get_result()->fetch_assoc();

// Esercizi di oggi dalla scheda attiva
$esercizi_oggi = [];
if ($scheda) {
    $id_scheda = (int)$scheda['id_scheda'];
    $qe = $db->prepare("
        SELECT e.nome, se.serie, se.ripetizioni, se.peso_kg,
...
<div class="card">
    ...
    <?php if (empty($esercizi_oggi)): ?>
        <div class="empty-state">
            ...
            <p>Nessun esercizio previsto per oggi.<br>
               <?= $scheda ? 'Giorno di riposo — goditi il recupero!' : 'Crea la tua scheda per iniziare.' ?>
            </p>
        </div>
        <?php if (!$scheda): ?>
        <a href="/palestra/view/scheda.php" class="btn-start">+ CREA LA TUA SCHEDA</a>
        <?php endif; ?>

    <?php else: ?>
        <?php
        $tipo_icon = ['forza' => '💪', 'cardio' => '🔥', 'mobilita' => '🤸'];
        ?>
        <div class="exercise-list">
            <?php foreach ($esercizi_oggi as $i => $ex):
                $dettaglio = $ex['serie'] . 'x' . $ex['ripetizioni'];
                if ($ex['peso_kg']) $dettaglio .= ' · ' . $ex['peso_kg'] . ' kg';
                $muscolo = ucfirst(str_replace('_', ' ', $ex['gruppo_muscolare']));
                $icona   = $tipo_icon[$ex['tipo']] ?? '💪';
            ?>
            <div class="exercise-item">
                <div class="exercise-num"><?= $i + 1 ?></div>
                <div class="exercise-info">
                    <div class="exercise-name"><?= htmlspecialchars($ex['nome']) ?> <?= $icona ?></div>
                    <div class="exercise-detail"><?= $dettaglio ?></div>
                </div>
                <div class="exercise-muscle"><?= $muscolo ?></div>
            </div>
            <?php endforeach; ?>
        </div>

        <a href="/palestra/view/allenamento.php" class="btn-start">▶ AVVIA ALLENAMENTO</a>
    <?php endif; ?>
</div>
